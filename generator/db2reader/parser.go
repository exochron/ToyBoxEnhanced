...
type wdc3_source struct {
	...
	hotfixed_records map[int32][]byte
}
...
func (w wdc3_source) fetch_record(record_id int32) []byte {
	if record, ok := w.hotfixed_records[record_id]; ok {
		return record
	}

	entry := w.id_map[record_id]
...
func prepareWDC3(w WDC3, hotfix XFTH) wdc3_source {
	...
	obj.hotfixed_records = make(map[int32][]byte)
	for _, block := range hotfix.Blocks {
		if block.table_hash == w.Header.table_hash {
			obj.hotfixed_records[int32(block.record_id)] = block.record
		}
	}
