...
function TBE_ToySpellButton_UpdateButton(self)
    ...
    local itemIndex = (ToyBox.TBEButtons.PagingFrame:GetCurrentPage() - 1) * ADDON.TOYS_PER_PAGE + self:GetID();
...
local function UpdateButttons()
    for i = 1, ADDON.TOYS_PER_PAGE do
        local button = ToyBox.TBEButtons["spellButton" .. i];
...
ADDON:RegisterLoadUICallback(function()
    ...
    ToyBox.TBEButtons = CreateFrame("Frame", nil, ToyBox, "TBEButtonFrameTemplate")
    ToyBox.TBEButtons:SetFrameStrata("DIALOG")
    ToyBox.TBEButtons.OnPageChanged = function(userAction)
    ...
    hooksecurefunc("ToyBox_UpdatePages", function()
        ...
        ToyBox.TBEButtons.PagingFrame:SetMaxPages(maxPages)
    end)
    hooksecurefunc("ToyBox_UpdateButtons", UpdateButttons)
    ToyBox:HookScript("OnMouseWheel", function(self, value)
        ToyBox.TBEButtons.PagingFrame:OnMouseWheel(value);
    end)

    ToyBox.TBEButtons:RegisterEvent("PLAYER_REGEN_ENABLED")
    ToyBox.TBEButtons:RegisterEvent("PLAYER_REGEN_DISABLED")
    ToyBox.TBEButtons:SetScript("OnEvent", function(self, event, arg1)
        self:SetShown(event == "PLAYER_REGEN_ENABLED")
    end)

    ToyBox.TBEButtons:SetShown(not ADDON.inCombat)
